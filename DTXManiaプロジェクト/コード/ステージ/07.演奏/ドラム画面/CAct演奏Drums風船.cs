...
namespace DTXMania
{
    internal class CAct演奏Drums風船 : CActivity
    {
        ...
        public int On進行描画( int n連打ノルマ, int n連打数 )
        {
            ...
            if( n連打数 != 0 )
            {
                if( this.txキャラクター != null )
                    this.txキャラクター.t2D描画( CDTXMania.app.Device, 240, 0 );
                for( int i = 0; i < 5; i++ )
                {
                    if( n残り打数[ i ] < n連打数 )
                    {
                        if( this.tx風船枠[ i ] != null )
                            this.tx風船枠[ i ].t2D描画( CDTXMania.app.Device, 398 + ( this.ct風船ふきだしアニメ.n現在の値 == 1 ? 3 : 0 ), 115 );
                        break;
                    }
                }
                if( this.tx連打枠 != null )
                    this.tx連打枠.t2D描画( CDTXMania.app.Device, 383, 31 );
                this.t文字表示( 424 + 62, 58, n連打数.ToString(), n連打数 );
            ...
            if( n連打数 == 0 && CDTXMania.stage演奏ドラム画面.actChara.b風船連打中 )
            {
                ...
                else if( this.ct風船終了.b進行中 )
                {
                    ...
                    if( this.ct風船終了.b終了値に達した )
                    {
                        ...
                        CDTXMania.stage演奏ドラム画面.b連打中 = false;
                    }
                }
                CDTXMania.stage演奏ドラム画面.actChara.b風船連打中 = false;
