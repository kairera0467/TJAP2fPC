        ...
        public virtual void Start( int player )
		{
            ...
            for (int i = 0; i < 128; i++)
            {
                if(!RollCharas[i].IsUsing)
                {
                    ...
                    RollCharas[i].Type = random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Ptn);
                    RollCharas[i].OldValue = 0;
                    RollCharas[i].Counter = new CCounter(0, 5000, 1, CDTXMania.Timer);
                    if (CDTXMania.stage演奏ドラム画面.bDoublePlay)
                    {
                        switch (player)
                        {
                            case 0:
                                RollCharas[i].X = CDTXMania.Skin.Game_Effect_Roll_StartPoint_1P_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_1P_X.Length)];
                                RollCharas[i].Y = CDTXMania.Skin.Game_Effect_Roll_StartPoint_1P_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_1P_Y.Length)];
                                RollCharas[i].XAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_1P_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_1P_X.Length)];
                                RollCharas[i].YAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_1P_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_1P_Y.Length)];
                                break;
                            case 1:
                                RollCharas[i].X = CDTXMania.Skin.Game_Effect_Roll_StartPoint_2P_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_2P_X.Length)];
                                RollCharas[i].Y = CDTXMania.Skin.Game_Effect_Roll_StartPoint_2P_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_2P_Y.Length)];
                                RollCharas[i].XAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_2P_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_2P_X.Length)];
                                RollCharas[i].YAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_2P_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_2P_Y.Length)];
                                break;
                            default:
                                return;
                        }
                    ...
                    {
                        RollCharas[i].X = CDTXMania.Skin.Game_Effect_Roll_StartPoint_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_X.Length)];
                        RollCharas[i].Y = CDTXMania.Skin.Game_Effect_Roll_StartPoint_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_StartPoint_Y.Length)];
                        RollCharas[i].XAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_X[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_X.Length)];
                        RollCharas[i].YAdd = CDTXMania.Skin.Game_Effect_Roll_Speed_Y[random.Next(0, CDTXMania.Skin.Game_Effect_Roll_Speed_Y.Length)];
                    }
                    break;
                }
            }
			...
			if( !base.b活性化してない )
			{
                ...
                for (int i = 0; i < 128; i++)
                {
                    if(RollCharas[i].IsUsing)
                    {
                        ...
                        CDTXMania.Tx.Effects_Roll[RollCharas[i].Type]?.t2D描画(CDTXMania.app.Device, RollCharas[i].X, RollCharas[i].Y);
                        // 画面外にいたら描画をやめさせる
                        if(RollCharas[i].X < 0 - CDTXMania.Tx.Effects_Roll[RollCharas[i].Type].szテクスチャサイズ.Width || RollCharas[i].X > 1280)
                        {
                            RollCharas[i].Counter.t停止();
                            RollCharas[i].IsUsing = false;
                        }
                        if (RollCharas[i].Y < 0 - CDTXMania.Tx.Effects_Roll[RollCharas[i].Type].szテクスチャサイズ.Height || RollCharas[i].Y > 720)
                        {
                            RollCharas[i].Counter.t停止();
                            RollCharas[i].IsUsing = false;
                        }
