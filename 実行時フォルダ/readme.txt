...
・風船連打が叩けるようになるタイミングは、発声時刻から17ミリ秒前からです。
...
○見た目が中途半端なところ
・譜面分岐のアニメーションが中途半端に新筐体のものだったり。
　・ver2015060100から7～14っぽいアニメーションも追加しました。
・風船連打のアニメーションが中途半端。
　・画像作成がめんどい
・炎画像が結構適当
...
・拡張性に欠けてます。
　・特に2016081100に追加した画像は拡張性0です。そのうち改善します。
...
　・なので一部の曲ではこのソフトのために加工する必要があります。
　　・「DADDY MULK」の場合は連打中に1小節はさむ、など。
・曲名はTJAから読み込みます。DTXファイルに曲名書く必要が無かったりします。
...
・連打音符の表示優先度は、全コース共通で「通常音符より上」です。
...
○再配布に関して
本体の再配布、逆アセンブルは自由です。
ただし「本家っぽい背景モード」を有効にした状態の本体の配布は一切禁止します。また、各種隠しコマンドを公開することを禁じます。
(※動画に関しては特に制限はありません。)
再配布をする際は、Licensesフォルダの同梱を忘れないようにしてください。


○画像の再配布について
・私が作成した画像の再配布・別用途での利用などは、自由にしてくださっても結構です。
・私以外が作成したものについては、TJAPlayer2 for.PC(仮)以外のソフトでの使用を前提とした再配布・改造をする際は製作者にお問い合わせください。
　・2_background.jpg		メロディー氏
　・7_combo taiko.png		メロディー氏(2016081100以降)
　・7_combo taiko_large.png	メロディー氏(2016081100以降)
　・Chara/clear_(n).png		Music Revolver 76573氏
　・その他DTXManiaの画像	RWS氏、GT氏など

・スキン配布は、できるだけデフォルトスキンの差分にしていただけるとありがたいです。
...
○更新履歴
2017050800	デフォルトスキンのフォルダ名を「Default」に変更。
		隠しコマンドの追加。(実験的)
2017041600	デフォルトスキンの画像が数枚抜けていたので追加
		一度再生したコンボ音声が演奏内で鳴らなくなっていた不具合を修正。
		エラー防止のため、DTXVモード・コンパクトモードに侵入しないようにした。
2017031800	config.iniから判定範囲を変更できるようにした。
		設定画面のヘッダー画像、フッター画像を読み込めるようにした。
		デフォルトスキンのいらんやつを数枚削除。
2017012400	演奏中の一時停止メニューを簡易実装。
		...
		readmeを一部書き直し。
2016092600	DEBUGビルド設定を外して再ビルド。
		DEBUGビルドの場合、タイトル画面で表示するようにした。
2016092400	サンプル譜面を追加。
